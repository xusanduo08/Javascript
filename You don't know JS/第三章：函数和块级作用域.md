...
我们在for循环的头部声明了变量`i`，我们的意图很明显----只在for循环的作用域内使用这个变量`i`。这个变量其实是处在外部作用域的（外部函数或者全局作用域），我们先忽略。
...
但或许更重要的时，开发者更喜欢手动去避免在预期作用域之外使用目标变量，比如在错误的地方使用一个不存在的变量这将会导致错误发生。一个真正的块级作用域能确保`i`只存在于for循环内部，而如果在函数的其他位置使用`i`则会引起错误。这能确保变量不会被混乱的复用以至于代码难以维护。
...
我们在第二章学习过`with`。虽然难以理解，但它是块级作用域的一个例子。根据传入的对象创建的作用域只存在于`with`的声明中，在外部作用域不存在。
...
通过清晰的代码块来创建块级作用域能解决上面的一些问题，还使代码的附属关系更加清晰。清晰的代码相比隐晦的或者虽然精巧但不清晰的代码来说更受欢迎。这种显式创建块级作用域的方式很容易实现，而且也和其他语言中块级作用域的工作方式相契合。
...
可以通过使用一对大括号`{...}`将`let`包裹的方式来给`let`声明指定绑定的作用域，只要当前的声明是有效的。上面的代码中，我们在 if 声明内部显式的创建了一个代码块，这会使后期重构代码时迁移代码变得容易，而且还不会影响外围作用域中的if声明的位置和语义。

**注意**：查看附录B来获知另一种显式创建块作用域的方法。

第四章我们将会讨论提升。提升是指声明被认为其存在于包含它的整个作用域。
...
块作用域的另一个用处于闭包和垃圾回收释放内存有关。这里我们做简单介绍，关于闭包的详细内容将在第五章中介绍。
...
点击事件的回调函数`click`方法不需要`someReallyBigData`变量。也就是说，理论上在`process(...)`运行结束后，内存占用很多的数据结构就可以被垃圾回收了。然而，大多数情况下（根据具体实现）JS引擎依然会保留相关数据，因为`click`方法对整个作用域有个闭包。
...
`let`不仅将`i`绑定到了for循环体，还将`i`绑定到了循环的每个迭代中，并确保在前一个迭代完成后对`i`重新赋值。
